...
const uploadTSMData = async (req, res) => {
  try {
    ...
    await db.execute(
      "INSERT INTO TSM_kml (location, state, sector_id, kml_file) VALUES (?, ?, ?, ?)",
      [location, state, sector_id, kml_file]
    );

    res.status(201).json({ message: "TSM data uploaded successfully." });
  } catch (error) {
